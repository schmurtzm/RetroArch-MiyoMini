...
SDL_CONFIG	?= $(TOOLCHAIN_DIR)/bin/sdl-config
FREETYPE_CONFIG	?= $(TOOLCHAIN_DIR)/bin/freetype-config
...
DEF_FLAGS += -I. -Ideps -Ideps/stb -DMIYOO=1 -DMIYOOMINI -DDINGUX -MMD
DEF_FLAGS += -Wall -Wno-unused-variable $(LTO)
DEF_FLAGS += -std=gnu99 -D_GNU_SOURCE
LIBS := -ldl -lz -lrt -pthread -lmi_sys -lmi_gfx
CFLAGS := 
